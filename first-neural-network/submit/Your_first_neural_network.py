...
# statistic every data's max cnt in train data
dates_set = set()
train_data = rides.iloc[features.index]
all_train_dates = train_data['dteday']
for day in all_train_dates:
    dates_set.add(day)

dates_set = list(dates_set)
date_cnt_map = {}
for date, cnt in train_data[['dteday','cnt']].values:
    date_cnt_map.setdefault(date, 0)
    if date in dates_set:
        date_cnt_map[date] = max(date_cnt_map[date], cnt)
cnts = [x for x in date_cnt_map.values()]
cnts = np.array(cnts)
gt_cnt = np.sum(cnts>250)
ls_cnt = np.sum(cnts<250)
print(gt_cnt/ls_cnt)
...
# 
# #### 请将你的答案填写在下方
# ~~1. 固定hidden_nodes（10）和learn_rate（0.05），增加训练次数（1000->2000），损失会减少，结果会表现出比较强的过拟合，表现为在车使用比较多的时候，预测比较准，但是车使用少的时候，预测要多很多~~
# ~~2. 固定iterations（1000）和learn_rate（0.05），增加hidden_node（10->20->30），损失由小变大，结果会表现出20的时候，效果比较好~~
# ~~3. 固定iterations（1000）和hidden_node（20），增加learn_rate（0.05->0.1->0.5），损失变小，但是曲线变得不再平滑，比较震荡，~~
# 当迭代次数大于5000之后，效果不会显著的增加了
# 
# #### 预测的效果
# 对于使用流量比较多的情况，网络拟合的较好，但是流量较少的情况下，网络拟合的不是很好。
# 
# #### 哪里出现了问题及为何出现问题
# 目前发现的可能性问题就是数据的分布，可以看到，验证集的date都是在冬天，冬天骑车量会比较少，因此将每天的最大流量取出并进行对比，发现在250以上的数据比250以下的数据量要多很多，大概是6倍的关系，整个数据集中流量较多的数据为14500+， 而流量较少的数据为2400+，导致流量较小的时候的学习不是很充分，因此较难拟合流量较小的情况。
# 
